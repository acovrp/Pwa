...
18. **Amazon Ads MCP queries** — for live campaign structure, budget, state: use `amzn-ads-mcp` → `query_campaign` directly. No CSV needed. Token expires hourly — **auto-refreshes via Stop hook at session end** (since 2026-06-01), so next session always starts connected. If auth error mid-session: run `refresh_ads_mcp_token.py` + restart. **Reporting tools (`create_report`, `create_campaign_report`, `create_product_report`) are broken on EU endpoint** — use `query_ads` on `wow_data.json` for ACOS/spend history, or `pull_ads_weekly.py` for fresh dashboard pulls.
